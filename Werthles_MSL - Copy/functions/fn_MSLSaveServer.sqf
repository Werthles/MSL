...
//units
{
	...
	["write", ["UNIT" + (str _forEachIndex), "isPlayer", (isPlayer _x)]] call _inidbi;
	if (isMultiplayer) then {
		["write", ["UNIT" + (str _forEachIndex), "isPlayable", (_x in playableUnits)]] call _inidbi;
	} else {
		["write", ["UNIT" + (str _forEachIndex), "isPlayable", (_x in switchableUnits)]] call _inidbi;
	};
	["write", ["UNIT" + (str _forEachIndex), "skill", (skill _x)]] call _inidbi;
	["write", ["UNIT" + (str _forEachIndex), "rank", (rank _x)]] call _inidbi;
	_x setVariable ["MSLID",_forEachIndex];
	if (vehicle _x != _x) then {
		if ((vehicle _x) in (allMissionObjects "air")) then {
			["write", ["UNIT" + (str _forEachIndex), "vehicleType", "AIR"]] call _inidbi;
		};
		if ((vehicle _x) in (allMissionObjects "boat")) then {
			["write", ["UNIT" + (str _forEachIndex), "vehicleType", "BOAT"]] call _inidbi;
		};
		if ((vehicle _x) in (allMissionObjects "cars")) then {
			["write", ["UNIT" + (str _forEachIndex), "vehicleType", "CAR"]] call _inidbi;
		};
		["write", ["UNIT" + (str _forEachIndex), "vehicle", (vehicle _x) getVariable ["MSLID",0]]] call _inidbi;
		["write", ["UNIT" + (str _forEachIndex), "vehiclePosition", (vehicle _x) getCargoIndex _x]] call _inidbi;
	};
}forEach allUnits;
