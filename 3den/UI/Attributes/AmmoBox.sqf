...
switch _mode do {
	case "onLoad": {
		...
		_entity = get3DENSelected "object" select 0;//if (count _selectedObjects > 1) then {objnull} else {get3DENSelected "object" select 0};
		...
		if (isnil "AmmoBox_list") then {
			...
			_magazines = []; //--- Store magazines in an array and mark duplicates, so nthey don't appear in the list of all items
			{
				...
				if (_weaponTypeCategory != "VehicleWeapon" && _weaponTypeID >= 0) then {
					...
					_listType = _list select _weaponTypeID;
					if (_weaponPublic) then {
						...
						_listType pushback [_displayName,_weapon,_picture,_type,false];
					};
					...
					if (_weaponPublic || _weapon in ["throw","put"]) then {
						{
							...
							{
								_mag = tolower _x;
								if ({(_x select 1) == _mag} count _listType == 0) then {
									...
									if (getnumber (_magCfg >> "scope") == 2) then {
										...
										_listType pushback [_displayName,_mag,_picture,2,_mag in _magazines];
			...
			//--- Backpacks
			{
				...
				if (_weaponTypeID >= 0) then {
					...
					if (getnumber (_weaponCfg >> "scope") == 2) then {
						...
						_picture = gettext (_weaponCfg >> "picture");
						_listType = _list select _weaponTypeID;
						_listType pushback [_displayName,_weapon,_picture,3,false];
					};
				};
...
*/
	...
	case "filterChanged": {
		_ctrlFilter = _params select 0;
		_cursel = if (count _params > 1) then {_params select 1} else {AmmoBox_filter};
		AmmoBox_filter = _cursel;

		_ctrlGroup = ctrlparentcontrolsgroup _ctrlFilter;
		_ctrlList = _ctrlGroup controlsGroupCtrl 101;
		...
		if (_cursel > 0) then {
			...
			{_items = _items + _x;} foreach _list; //--- Process all items, and later pick the ones which are in the box
			_items sort true;
		};
			...
			_types = _x;
			{
				...
				if (_type in _types && (!_isDuplicate || _cursel > 0)) then {
					...
					if ((_cursel == 0 && _value != 0) || (_cursel > 0)) then {
						...
						_lnbAdd = _ctrlList lnbaddrow ["",_displayName,_valueText,""];
